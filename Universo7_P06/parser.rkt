...
(define (parse sexp)
  (cond
      ...
      [(list? sexp)
         (case (car sexp)
            [(+ - * - / % max min pow < > <= >= /= = not and or zero? head tail empty?) ; para operaciones
               (opS
...
(define (getName lst)
  (match lst
    ['() '()]
    [(cons (bindingS name value) xs) (cons name (getName xs))]))
...
(define (getExpr lst)
  (match lst
    ['() '()]
    [(cons (bindingS name value) xs) (cons (desugar value) (getExpr xs))]))

;; Función auxiliar para el desugar de bindings.
;; desugarBind : list -> list
;; Función auxiliar para obtener las expresiones de los bindings.
;; getName : list -> list
(define (desugarBind lst)
  (match lst
    ['() '()]
    [(cons (binding name value) xs) (cons (binding name (desugar value)) (desugarBind xs))]))
...
(define (desugar expr)
  (match expr
      ...
      [(boolS b) (bool b)]
      ; Eliminamos el azúcar sintáctica de cada uno de los elementos
      [(listS elem) (lisT (map desugar elem))]
      ; Eliminamos el azúcar sintáctica de los bindings y el body
      [(recS binds body) (rec (desugarBind binds) (desugar body))]
